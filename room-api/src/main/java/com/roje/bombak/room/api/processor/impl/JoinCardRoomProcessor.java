 ...
 **/
@Slf4j
public class JoinCardRoomProcessor<P extends Player,R extends Room<P>> implements CommonProcessor {

    private final RoomManager<P,R> roomManager;

    private final RoomMessageSender sender;

    public JoinCardRoomProcessor(RoomManager<P, R> roomManager, RoomMessageSender sender) {
    ...
    public void process(InnerClientMessage message) throws Exception {
        ...
        R room = roomManager.getCardRoom(joinRoomReq.getRoomId());
        if (room == null) {
            log.info("没有找到房卡房{}", joinRoomReq.getRoomId());
